...
from committee.debate.policies.allocation import per_round_share, split_evenly, uniform_tiers
...
class UniformPolicy(BudgetPolicy):
    ...
    def allocate(self, state: DebateState | None, ledger: Ledger, round: int, lenses: list[str]) -> BudgetDecision:
        ...
        research = split_evenly(per_round_share(ledger, Pool.RESEARCH, settings.explore_research_frac, round), lenses)
        argue = split_evenly(per_round_share(ledger, Pool.DEBATE, settings.explore_argue_frac, round), lenses, floor=settings.argue_floor_tokens)
